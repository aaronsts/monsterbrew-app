...
- **The picker spans both your saved creatures and the SRD bestiary** (added at the user's request). Yours come first, most recently touched first, each row marked `MINE`; a `Source` dropdown narrows to one side. Picking any row — including your own — loads a fresh **unsaved copy** with the stored id stripped by `starterFromEntry`, so saving makes a second creature. That is what separates it from the Recent list, which opens a creature for editing in place.
